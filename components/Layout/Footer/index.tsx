...
import { footerlabels } from '@/app/landpage/api/data'
...
const Footer: FC = () => {
  return (
    <footer className='pt-16 bg-black max-w-7xl mx-auto px-4 flex items-center justify-between'>
      <div className='container px-4'>
            ...
            <div className='flex gap-6 items-center relative z-1'>
              <Link href='https://www.facebook.com/' className='group'>
                <Icon
                  ...
                  width='24'
                  height='24'
                  className='text-white group-hover:text-[#2D6A4F]'
                />
              </Link>
            ...
            <ul>
              {headerData.map((item, index) => (
                <li key={index} className='pb-4'>
                  <Link
                    href={item.href}
                    className='text-white/60 hover:text-[#2D6A4F] text-17'>
                    {item.label}
                  </Link>
                </li>
              ))}
        ...
        <p className='text-white/40 text-center py-8 border-t border-white/10'>Design & Develop by <Link className='hover:text-[#2D6A4F]' href={"https://getnextjstemplates.com/"}>GetNextjsTemplates.com</Link></p>
      </div>
    </footer>
  )
}
