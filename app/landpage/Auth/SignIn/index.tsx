  ...
  return (
    <>
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className='mb-9'>
          <button
            onClick={loginUser}
            type='submit'
            className='bg-[#a2e8bc] w-full py-3 rounded-lg text-lg font-medium border border-primary hover:text-[#2D6A4F] hover:bg-transparent'>
      ...
      <Link
        ...
        className='mb-2 inline-block text-base text-dark hover:text-[#2D6A4F] text-white dark:hover:text-[#2D6A4F] text-center'>
        Forgot Password?
      </Link>
      <p className='text-body-secondary text-white text-base text-center'>
        Not a member yet?{' '}
        <Link href='/' className='text-[#2D6A4F] hover:underline'>
          Sign Up
        </Link>
      </p>
    </>
